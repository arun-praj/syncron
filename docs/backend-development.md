...
Disconnected memberships reserve capacity for a 30-second reconnect grace so page and extension reloads can reconnect; after the grace they close with `DISCONNECTED_TIMEOUT`. Explicit leave, kick, or room end closes memberships immediately. Playback remains transient across an API restart, while the persisted room and active memberships remain recoverable during the grace period. Host failover selects the earliest currently connected user after the grace period, with user ID as the tie-break. One local mutation queue serializes coordinator changes; the `RoomCoordinator` interface is the boundary for a future Durable Object implementation.
