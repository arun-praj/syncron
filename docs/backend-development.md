...
Disconnected memberships reserve capacity until explicit leave, kick, or room end so page and extension reloads cannot remove party members. Playback remains transient across an API restart, while the persisted room and active memberships remain recoverable. Host failover selects the earliest currently connected user after the existing grace period, with user ID as the tie-break. One local mutation queue serializes coordinator changes; the `RoomCoordinator` interface is the boundary for a future Durable Object implementation.
